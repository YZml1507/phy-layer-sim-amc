...
1. 时域均衡器要解 $(L_h+L_w)$ 阶的线性系统，复杂度随信道长度增长；
2. 信道频谱的深凹点上，MMSE 也只能折中——ISI 消了，该频点信噪比还是差。

OFDM 换了一条更聪明的思路：**不做时域均衡，让 ISI 在结构上消失**。原理
分两步：

- **拉长符号周期**：把 64 个 QAM 符号并排放在 64 个正交子载波上，一个
  OFDM 符号持续 $64$ 个采样——多径那点儿拖尾相对超长符号周期变得可以
  忽略；
...
相当于合法循环移位，被吸收进 $H$ 的线性相位项），但绝不能**晚**
（窗尾会抓到下一个符号的 CP 样本，不属于本符号的循环延拓 → ISI 直接
...
噪声区 $M\approx0$；进入前导后两半相关取共轭相消、$M\to1$ 形成
**平台**。平台宽约等于 CP 长——粗定时够了，精确点还得下一步。
...
CFO 估计有残差，随时间累积成相位斜坡；定时抖动也引入公共相位旋转。
...
# ... 过信道 + 噪声 ...
syms = ofdm_rx(rx_wave)       # S&C粗定 -> 匹配滤波首径 -> CFO -> derotate
                              # -> FFT窗(偏早2) -> LS估计+时域降噪
                              # -> 逐符号 CPE 校正 -> 除以 H_hat
```
...
残余 ISI+ICI——本项目实测：抽头 8、CP 16 安全，抽头拉到 15 以上任何
同步位置都救不回来。
...
**Q：FFT 窗偏了会怎样？**
A：偏早=合法（窗内都是本符号循环延拓，表现为 $H$ 乘线性相位项）；
